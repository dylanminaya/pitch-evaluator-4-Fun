...
// Envía el correo con el enlace para aceptar la invitación.
export async function sendOrganizerInvitationEmail(params: {
  ...
  // Resend envía el correo usando el remitente configurado en .env.
  const { data, error } = await resend.emails.send({
    from: env.EMAIL_FROM,
    to,
    subject: `Invitación para organizar ${eventName}`,
